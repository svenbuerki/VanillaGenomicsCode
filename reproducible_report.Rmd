...
This project relies on paired-end Illumina short-read data (2x150bp) stored in `fastq` format and compressed using `gz` protocol. The data produced in this project are deposited on NCBI under BioProject (pending) and SRA accessions XXX-ZZZ. We are also using data from BioProject [PRJNA633886](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA633886/) published in @Hasing2020.

<!--- PAIGE: Please provide data to include a Table with samples information -->
...
4. Genome size and heterozygosity.
5. Ploidy level and genome complexity.
6. Reconstructing genome sequences.
7. Comparative chromosome level analyses.
...
# Ploidy level and genome complexity
...
# Comparative chromosome level analyses

We applied the same approach than @Shirasawa2022 and compared chromosome-level genome sequences of *Vanilla planifolia* samples with `Minimap2` [@Li2018], and produced coverage maps with the R package [`pafr`](https://dwinter.github.io/pafr/index.html). Coverage maps allowed assessing for any chromosomal re-arrangements as well as evaluating coverage of our target sample compared to the reference genome. To further investigate putative chromosomal rearrangements, we inferred dot plots with the R package [`pafCoordsDotPlotly`](https://github.com/tpoorten/dotPlotly). Finally, a heatmap showing percentage of identities between each sample and the reference genome at chromsome level was inferred using a custom `R` script. All these comparative analyses rely on the output of the `Minimap2` analyses, which are in [`paf`](https://cran.r-project.org/web/packages/pafr/vignettes/Introduction_to_pafr.html#:~:text=PAF%20is%20a%20plain%20text,least%20the%20following%2012%20columns.&text=In%20addition%2C%20each%20row%20can,the%20SAM%20specification%20(pdf)) format. The code to execute these analyses is presented below (if you need to install software see below):

## Code for full genome alignments
...
# 3. Load package
# Warning: Please install pafr with devtools prior to executing this code (see below)
...
Since the previous analyses did not unveil any chromosomal rearrangements, we can produce a heatmap to compare percentage identities between chromosomes (against the reference). To do that, we need to start by inferring the percentage identities between each target chromosome and the reference as follows: `100*(nmatch/alen)` (see [here](https://cran.r-project.org/web/packages/pafr/vignettes/Introduction_to_pafr.html#:~:text=PAF%20is%20a%20plain%20text,least%20the%20following%2012%20columns.&text=In%20addition%2C%20each%20row%20can,the%20SAM%20specification%20(pdf)) for more details). The custom `R` code below (relying on base `R` functions) builds a matrix storing percentage identities for all samples, which will then be used as input to infer the heatmap.
...
The protocol to install `minimap2` is described [here](https://github.com/lh3/minimap2). Apart from `pafCoordsDotPlotly.R` (see below), the other R packages can be installed as follows:

```{r eval=FALSE, echo = T}
# 1. Install pafr with devtools
# If you don't have devtools do:
#install.packages(devtools)
devtools::install_github("dwinter/pafr")

# 2. Install gplots from CRAN
