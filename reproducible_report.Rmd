...
## SNP calling and clustering analyses {#SNPcallPCA}

To further compare the genetic structure of the Mexican *Vanilla* samples, we have called and filtered SNPs across samples. These data were used to conduct a principal component analysis (PCA) allowing assessing genetic structuring within samples (compared to the reference genome).  
...
Merge all the `bam` files from the genome reconstruction analyses (stored in the `Bowtie` folder) and use `bcftools` to call and filter variants (SNPs). Further variant filtering is also done in R (see below) prior to inferring the PCA.
...
# The total number of SNPs: 5905551 
# SNP genotypes are stored in SNP-major mode (Sample X SNP).

# 8. Open file
genofile <- SNPRelate::snpgdsOpen("SNPs/calls.gds")

# 9. Create a data.frame with SNP information
...
# 10. Prune SNP set to account for linkage disequilibrium and perform PCA
...
for(i in 1:length(LDthresh)){
  ...
  snpset <- SNPRelate::snpgdsLDpruning(genofile, ld.threshold=as.numeric(LDthresh[i]), autosome.only = FALSE) # Output = SNP.ID
  ...
  target.samples <- c("Van31.bam", "Van67.bam", "Van26.bam")
  ...
  pdf(pdfFile)
  #Plot PCA
  plot(tab$EV1, tab$EV2, xlab=paste0("Eigen vector 1 (", pc.percent[1], "%)"), ylab=paste0("Eigen vector 2 (", pc.percent[1], "%)"), pch = 19, main = paste0("LD threshold of ", LDthresh[i], " resulting in ", NSNPs, " SNPs"))
...
# 10. Infer SNP density map
# - This needs info on length of chromosomes
...
#Van$Chromosome <- gsub("[.]1", "", gsub("CM0", "", Van$Chromosome))
#SNPslidingdat$Chromosome <- gsub("[.]1", "", gsub("CM0", "", SNPslidingdat$Chromosome))
SNPslidingdat <- SNPslidingdat[-which(SNPslidingdat$SNP_nbr == 0),]
#Plot data
out.file <- "RCircosSagebrushGenome.pdf";
...
A total of 5,905,551 bi-allelic SNPs were called across the 15 Mexican *Vanilla* samples. When a LD threshold of 0.2 was set 7,290 SNPs remained, whereas when it was set to 0.8, 360,942 SNPs remained post filtering. However, the general clustering between these two extreme values led to highly similar clustering (see Figure \@ref(fig:PCASNPs)).
